...
def _safe_id(value: Any, fallback: str) -> str:
    text = _as_str(value, fallback).lower().strip()
    allowed = []
    for ch in text:
...
def _normalize_personality(card: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(personality, dict):
        ...
        return {"core": core, "flaws": flaws, "speech": speech}
    ...
    return {
        "core": traits or ["закрытый характер"],
        "flaws": ["не показывает слабость"],
        "speech": "живая, узнаваемая речь",
    }
...
def _normalize_character(card: Any, character_id: str, role_fallback: str = "npc") -> dict[str, Any]:
    ...
    name = _as_str(card.get("name"), f"Generated {character_id.replace('_', ' ').title()}")
...
def _normalize_story_plan(story_plan: Any, data: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, act in enumerate(act_structure[:4], start=1):
        if isinstance(act, dict):
            ...
            normalized_acts.append({
                "act": index,
                "goal": _as_str(act, f"Акт {index}"),
                "must_happen": ["развить конфликт"],
                "must_not_resolve_yet": ["главный конфликт"],
            })
    ...
    if isinstance(character_arcs, list):
        character_arcs = {
            f"arc_{i + 1}": {
                "start_point": _as_str(item),
                "pressure": "будет уточняться",
                "possible_direction": "без финального закрытия",
            }
            for i, item in enumerate(character_arcs)
        }
    elif not isinstance(character_arcs, dict):
        protagonist_id = data.get("protagonist_id") or (data.get("protagonist") or {}).get("id") or "pc_01"
        character_arcs = {
            protagonist_id: {
                "start_point": "входит в историю под давлением",
                "pressure": "внешнее давление и внутреннее выгорание",
                "possible_direction": "выстроить границы без мгновенного исцеления",
            }
        }
    ...
    if isinstance(relationship_focus, str):
        relationship_focus = [{
            ...
            "starting_dynamic": relationship_focus,
            "slow_burn_rule": "не решать отношения сразу",
        }]
    ...
    slots_list = _as_list(slots)
    normalized_slots = []
    defaults = [
        ("story_slot_1", "Напряжение", "внутреннее/внешнее давление", "старт"),
        ("story_slot_2", "Скрытый слой", "приближение мистического/сюжетного слоя", "старт"),
    ]
...
def _normalize_relationships(relationships: Any, characters: dict[str, Any], protagonist_id: str) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {}
    for raw_pid, rel in relationships.items():
        ...
        result[pid] = {
            **rel,
            ...
            "scores": rel.get("scores") if isinstance(rel.get("scores"), dict) else {
                "trust": 30,
                "tension": 40,
                "attachment": 20,
                "respect": 30,
            ...
            "a_view_of_b": rel.get("a_view_of_b") if isinstance(rel.get("a_view_of_b"), dict) else {
                "summary": "восприятие будет уточняться",
                "current_assumption": "может ошибаться",
            },
            "b_view_of_a": rel.get("b_view_of_a") if isinstance(rel.get("b_view_of_a"), dict) else {
                "summary": "восприятие будет уточняться",
                "current_assumption": "может ошибаться",
            },
...
def _normalize_current_state(current_state: Any, story_plan: dict[str, Any], protagonist_id: str) -> dict[str, Any]:
    ...
    return {
        **current_state,
        ...
        "environment": current_state.get("environment") if isinstance(current_state.get("environment"), dict) else {
            "light": "не указано",
            "sound": "не указано",
            "air": "не указано",
            "details": [],
...
def normalize_bootstrap_json(data: dict[str, Any]) -> dict[str, Any]:
    """Convert loose GPT bootstrap output into the strict v9 runtime shape.

    This is intentionally defensive: Custom GPT may send strings where the
    preview builder expects objects, arrays of strings where story_plan expects
    structured entries, or old relationship keys like from/to. The normalizer
    prevents 500 errors and gives the validator a stable shape to check.
    """
