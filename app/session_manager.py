...
FINAL_BOOTSTRAP_FILES = ["session.json", "user_request.json", "protagonist.json", "characters_index.json", "state/knowledge_index.json", "state/relationship_index.json", "story_plan.json", "current_state.json", "npc_state.json", "future_locks.json", "continuity.json", "scene_history.json", "turns.json", "characters/", "state/knowledge/", "state/relationship_pairs/"]
...
def _needs_questionnaire(request: CreateSessionRequest) -> bool:
    ...
    if raw in {"начнем", "начнём", "старт", "создай сессию", "новая сессия"}:
        return True
    meaningful = [request.setting_request.strip(), request.protagonist_request.strip(), (request.romance_request or "").strip(), (request.tone or "").strip()]
...
def build_setup_preview(bootstrap_json: dict[str, Any]) -> str:
    ...
    lines.append(f"- **Имя для сцены:** {_visible_name(pc_card)}")
    lines.append(f"- **Техническое имя в state:** `{_one_line(pc_card.get('name'))}`")
    ...
    if isinstance(appearance, dict) and appearance:
        ...
        for key in ["height", "build", "hair", "eyes", "face", "style"]:
            if appearance.get(key):
                lines.append(f"- **{labels.get(key, key)}:** {_one_line(appearance.get(key))}")
        lines.append("")
    lines.append("### Жизнь и стартовое положение")
    lines.append(f"- **Где стартуем:** {_one_line(current_state.get('location'))}")
    ...
    if not known_cards:
        ...
        for card in known_cards[:10]:
            ...
            lines.append(f"- `{cid}` — **{_visible_name(card)}** (`{_one_line(card.get('name'))}`), {_one_line(card.get('role'))}. {_one_line(card.get('past_short'), 'связь будет уточняться сценами')}")
    lines.append("")
    lines.append("### План новеллы без спойлеров")
    ...
    lines.append(f"- **Цель героини:** {_one_line(story_plan.get('player_goal'))}")
    lines.append(f"- **Главный конфликт:** {_one_line(story_plan.get('central_conflict'))}")
    lines.append(f"- **Вопрос истории:** {_one_line(story_plan.get('central_question'))}")
    lines.append(f"- **Стартовая сцена должна:** {_one_line(story_plan.get('opening_scene_intent'))}")
    ...
    lines.append("Напиши `подтверждаю`, если всё подходит. Или напиши, что исправить: героиню, жизнь/работу, знакомых, тон, романтику, сеттинг, стартовую сцену или план.")
    return "\n".join(lines)
...
class SessionManager:
    ...
    def create_session(self, request: CreateSessionRequest) -> dict[str, Any]:
        ...
        if request.mode == "gpt_actions" and _needs_questionnaire(request):
            return {"session_id": None, "status": "needs_questionnaire", "mode": request.mode, "bootstrap_prompt": None, "questionnaire": _questionnaire_text(), "files_created": []}
        ...
        if request.mode == "debug_stub":
            ...
            return {"session_id": session_id, "status": "active", "mode": request.mode, "bootstrap_prompt": None, "questionnaire": None, "files_created": BASE_FILES}
        created_at = now_iso()
        session = {"session_id": session_id, "title": request.title or "Untitled novella", "status": "bootstrap_pending", "engine_version": get_settings().engine_version, "created_at": created_at, "updated_at": created_at}
        empty_files = {"session.json": session, "user_request.json": user_request, "protagonist.json": {}, "characters_index.json": {"ids": []}, "state/knowledge_index.json": {"ids": []}, "state/relationship_index.json": {"pair_ids": []}, "story_plan.json": {}, "current_state.json": {}, "npc_state.json": {}, "future_locks.json": {}, "continuity.json": {}, "scene_history.json": [], "turns.json": []}
        ...
        return {"session_id": session_id, "status": "bootstrap_pending", "mode": request.mode, "bootstrap_prompt": prompt, "questionnaire": None, "files_created": list(empty_files.keys()) + ["characters/", "state/knowledge/", "state/relationship_pairs/", "pending_bootstrap_prompt.md"]}
    ...
    def _write_bootstrap_files(self, session_id: str, bootstrap_json: dict[str, Any]) -> list[str]:
        ...
        self.storage.write_json(session_id, "state/knowledge_index.json", {"ids": [cid for cid in knowledge.keys() if not cid.startswith("_")]})
    ...
    def save_bootstrap_preview(self, session_id: str, bootstrap_json: dict[str, Any]) -> dict[str, Any]:
        ...
        session["updated_at"] = now_iso()
        self.storage.write_json(session_id, "session.json", session)
        return {"message_to_user": preview, "session_id": session_id, "status": "bootstrap_review_pending", "must_show_to_user": True, "wait_for_confirmation": True, "next_user_action": "Напиши `подтверждаю`, если всё подходит, или скажи, что изменить.", "preview": preview, "user_visible_preview": preview, "can_confirm": True, "diagnostics": {"character_count": len(bootstrap_json.get("characters", {}) or {}), "relationship_count": len(bootstrap_json.get("relationships", {}) or {}), "knowledge_count": len(bootstrap_json.get("knowledge", {}) or {}), "normalized": True}}
