...
Порядок:
1. По SCENE_CONTRACT_JSON создай scene_response JSON.
2. НЕ показывай scene_response JSON пользователю.
3. Сразу вызови applyTurnResult с этим scene_response.
4. После успешного applyTurnResult финальный ответ пользователю = только scene.rendered_text.
5. Если applyTurnResult вернул ошибку — не показывай JSON, коротко объясни техническую ошибку.

Railway уже собрал нужную память. Не используй старый чат как источник канона.

scene_response JSON должен иметь:
response_version, player_input, scene(header/body/player_options/status_panel/relationships_panel/rendered_text),
summary, important_facts, witnesses, proposed_updates(scene_state_patch/relationship_patches/knowledge_patches/new_or_updated_characters), safety_checks.

Важно по типам:
- scene.header.inventory = строка, не массив.
- scene.relationships_panel элементы должны иметь label и value.
- knowledge_patches используют character_id, reason, source_in_scene. Не who/add/source.
- relationship_patches используют pair_id, change_type, entry, reason, source_in_scene.
...
- В видимой сцене имена можно писать кириллицей, если так задано пользователем.
...
- Proposed updates сохраняют только важное; не сохраняй весь диалог и мысли игрока как знания NPC.
""".strip()
...
def _compact_character_card(card: dict[str, Any]) -> dict[str, Any]:
    return {
        "id": card.get("id"),
        "name": card.get("name"),
        "display_name": card.get("display_name") or card.get("visible_name") or card.get("name_ru") or card.get("russian_name"),
        "role": card.get("role"),
        "age": card.get("age"),
...
def _compact_contract(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in contract.get("loaded_characters", []) or []:
        if isinstance(item, dict):
            ...
            loaded_characters.append({"character_id": item.get("character_id"), "display_name": item.get("display_name"), "load_reason": item.get("load_reason", []), "card": _compact_character_card(card)})
    ...
    for item in contract.get("recent_scene_history", [])[-3:]:
        if isinstance(item, dict):
            history.append({"turn": item.get("turn"), "summary": _clip_text(item.get("summary"), 350), "important_facts": _clip_list(item.get("important_facts", []), 5, 180), "witnesses": item.get("witnesses", [])})
    compact["recent_scene_history"] = history
    compact["output_requirements"] = {"tool_flow": "generate scene_response, call applyTurnResult, then show only scene.rendered_text"}
    return compact
...
def process_turn_gpt_actions(bundle: dict[str, Any], player_input: str) -> dict[str, Any]:
    ...
    return {"status": "gpt_actions_prompt_ready", "scene_prompt": prompt, "diagnostics": {"loaded_character_count": len(contract.get("loaded_characters", [])), "loaded_relationship_count": len(contract.get("loaded_relationships", [])), "visible_relationship_pair_ids": contract.get("visible_relationship_pair_ids", []), "compact_prompt_chars": len(prompt), "next_required_action": "generate scene_response internally, call applyTurnResult, then show only scene.rendered_text"}}
...
def process_turn_debug_stub(bundle: dict[str, Any], player_input: str) -> dict[str, Any]:
    ...
    scene_response = {"response_version": "novella.scene_response.v1", "player_input": player_input, "scene": {"header": {"story_title": story_title, "date": current_frame.get("date") or "День 1", "time": current_frame.get("time") or "время не задано", "location": current_frame.get("location") or "место сцены", "weather": current_frame.get("weather") or "атмосфера не задана", "scene_state": current_frame.get("scene_state") or "debug режим", "player_name": current_frame.get("player_display_name") or "Героиня", "visible_state": status.get("emotional_state", "нейтрально"), "outfit": current_frame.get("outfit") or "одежда не задана", "inventory": _as_list_text(current_frame.get("inventory"), "ничего не указано")}, "body": f"*{current_frame.get('player_display_name') or 'Героиня'} остаётся в точке: {current_frame.get('location') or 'место сцены'}.*", "player_options": {"thoughts": ["Отметить, что изменилось.", "Сдержать реакцию.", "Подумать, кому можно доверять."], "dialogue": ["Продолжим.", "Я хочу понять, что происходит.", "Не сейчас."], "actions": ["Осмотреться.", "Проверить вещи.", "Сделать шаг дальше."]}, "status_panel": {"hunger": status.get("hunger", "норма"), "fatigue": status.get("fatigue", "низкая"), "injuries": ", ".join(status.get("injuries", [])) if status.get("injuries") else "нет", "emotional_state": status.get("emotional_state", "нейтрально"), "skills": ", ".join(status.get("skills", [])) if status.get("skills") else "без активного ресурса", "custom": status.get("custom", [])[:2]}, "relationships_panel": [], "rendered_text": rendered}, "summary": "Тестовый ход debug_stub без настоящей генерации сцены.", "important_facts": [], "witnesses": current_frame.get("active_character_ids", []), "proposed_updates": {"scene_state_patch": {"scene_goal": "Продолжить сцену после последнего действия игрока.", "status": status}, "relationship_patches": [], "knowledge_patches": [], "new_or_updated_characters": []}, "safety_checks": {"used_only_loaded_characters": True, "respected_knowledge_boundaries": True, "no_hidden_future_reveal": True, "no_major_player_character_choice": True, "respected_player_input_order": True, "showed_only_scene_relationships": True, "header_has_no_focus_or_active_list": True, "notes": ["debug_stub не является полноценным писателем сцены"]}}
