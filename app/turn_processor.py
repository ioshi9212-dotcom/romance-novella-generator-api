...
- Варианты выбора должны быть социальными/эмоциональными/позиционными, а не квестовыми. Хорошо: поверить/не поверить, принять/оттолкнуть помощь, соврать, спросить прямо, задеть словами, выбрать чью сторону слушать. Плохо: нажать кнопку, подтвердить бронь, посчитать предметы, проверить интерфейс, выполнить инструкцию NPC.
...
Правила: не делай важный выбор за игрока; сохраняй порядок ввода; не раскрывай hidden future; сцена должна иметь meaningful beat; NPC действуют сами; proposed_updates сохраняют только важное. Нижний блок — короткая dashboard-панель, не объяснение сюжета.
""".strip()
