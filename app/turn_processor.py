...
Порядок: создай scene_response JSON по SCENE_CONTRACT_JSON, НЕ показывай JSON, сразу вызови applyTurnResult, после успеха покажи response.message_to_user.

Качество сцены:
- rendered_text должен быть полноценной сценой, не кратким пересказом.
- Body: 1800–3500 знаков, 5–9 плотных абзацев.
...
Правила: не делай важный выбор за игрока; сохраняй порядок ввода; NPC не читают мысли; не раскрывай hidden future; сцена должна иметь meaningful beat; proposed_updates сохраняют только важное.
""".strip()
