...
def _ensure_object_properties(schema_part: Any) -> None:
    if isinstance(schema_part, dict):
        if schema_part.get("type") == "object" and "properties" not in schema_part: schema_part["properties"] = {}
        for value in schema_part.values(): _ensure_object_properties(value)
    elif isinstance(schema_part, list):
        for item in schema_part: _ensure_object_properties(item)

def custom_openapi() -> dict[str, Any]:
    if app.openapi_schema: return app.openapi_schema
    schema = get_openapi(title=app.title, version=app.version, description="Railway API for generated novella sessions: preview gate, compact turn prompt, rendered scene response, state storage.", routes=app.routes)
    schema["servers"] = [{"url": RAILWAY_PUBLIC_URL, "description": "Railway production"}]
    for hidden_path in ["/api/v1/sessions/{session_id}/memory", "/api/v1/sessions/{session_id}/scene-contract", "/api/v1/sessions/{session_id}/bootstrap-result", "/api/v1/sessions/latest"]:
        schema.get("paths", {}).pop(hidden_path, None)
    _ensure_object_properties(schema); app.openapi_schema = schema; return app.openapi_schema
app.openapi = custom_openapi

def require_api_key(x_api_key: str | None = Header(default=None)) -> None:
    expected = get_settings().api_key
    if expected and x_api_key != expected: raise HTTPException(status_code=401, detail="Invalid or missing X-API-Key")

def _require_active_session(bundle: dict, action: str) -> None:
    status = (bundle.get("session") or {}).get("status")
    if status != "active": raise HTTPException(status_code=409, detail=f"Session must be active before {action}. Current status: {status}")

def _is_explicit_confirmation(text: str) -> bool:
    normalized = " ".join((text or "").strip().lower().replace("ё", "е").split())
    if not normalized: return False
    if any(marker in normalized for marker in ["не подтверждаю", "не сохраняй", "не запускай", "нет", "стоп", "правки", "исправь", "измени"]): return False
    if normalized.strip(".!") in {"подтверждаю", "ок", "okay", "ok", "сохраняй", "запускай", "подходит", "оставляем", "да"}: return True
    return any(phrase in normalized for phrase in ["да подтверждаю", "все подходит", "всё подходит", "можно сохранять", "можно запускать", "оставляем так", "подтверждаю все", "подтверждаю всё"])

@app.get("/health", operation_id="health")
def health() -> dict:
    settings = get_settings(); return {"status":"ok", "engine_version":settings.engine_version, "data_dir":str(settings.data_dir), "mode":"gpt_actions", "api_key_required":bool(settings.api_key)}
...
def create_session(request: CreateSessionRequest) -> dict: return SessionManager().create_session(request)
...
def get_start_questionnaire() -> dict:
    path = Path(__file__).resolve().parent.parent / "prompts" / "start_questionnaire.md"; return {"questionnaire": path.read_text(encoding="utf-8")}

@app.get("/api/v1/sessions", dependencies=[Depends(require_api_key)], operation_id="listSessions", include_in_schema=False)
def list_sessions() -> dict: return {"sessions": SessionManager().list_sessions()}

@app.get("/api/v1/sessions/latest", dependencies=[Depends(require_api_key)], include_in_schema=False)
def get_latest_session() -> dict:
    manager = SessionManager(); latest = manager.get_latest_session_id(prefer_active=True)
    if not latest: raise HTTPException(status_code=404, detail="No sessions found")
...
def get_session(session_id: str) -> dict:
    try: return SessionManager().get_memory(session_id)["session"]
...
def get_memory(session_id: str) -> dict:
    try: return SessionManager().get_memory(session_id)
    except FileNotFoundError as exc: raise HTTPException(status_code=404, detail=str(exc))

@app.post("/api/v1/sessions/{session_id}/bootstrap-result", dependencies=[Depends(require_api_key)], include_in_schema=False)
def apply_bootstrap_result_disabled(session_id: str) -> dict: raise HTTPException(status_code=410, detail="bootstrap-result is disabled in v9. Use bootstrap-preview and bootstrap-confirm.")
...
def create_bootstrap_preview(session_id: str, request: BootstrapPreviewRequest) -> dict:
    normalized_bootstrap = normalize_bootstrap_json(request.bootstrap_json); errors = validate_bootstrap_result(normalized_bootstrap)
    if errors: raise HTTPException(status_code=422, detail=errors)
    try: return SessionManager().save_bootstrap_preview(session_id, normalized_bootstrap)
    except FileNotFoundError as exc: raise HTTPException(status_code=404, detail=str(exc))
...
def confirm_bootstrap_preview(session_id: str, request: BootstrapConfirmRequest) -> dict:
    if not _is_explicit_confirmation(request.confirmation_text): raise HTTPException(status_code=409, detail="Bootstrap preview is not explicitly confirmed.")
    try: return SessionManager().confirm_bootstrap_preview(session_id)
    except FileNotFoundError as exc: raise HTTPException(status_code=404, detail=str(exc))
    except ValueError as exc: raise HTTPException(status_code=409, detail=str(exc))
...
def get_scene_contract(session_id: str) -> dict:
    ...
    try:
        manager = SessionManager(); bundle = manager.get_memory(session_id); _require_active_session(bundle, "building a scene contract"); return build_scene_contract(bundle)
    except FileNotFoundError as exc: raise HTTPException(status_code=404, detail=str(exc))
...
def process_turn(session_id: str, request: TurnRequest) -> dict:
    ...
    try: bundle = manager.get_memory(session_id); _require_active_session(bundle, "processing a turn")
    except FileNotFoundError as exc: raise HTTPException(status_code=404, detail=str(exc))
    if request.mode == "debug_stub":
        result = process_turn_debug_stub(bundle, request.player_input); apply_result = StateUpdater(manager.storage).apply_scene_response(session_id, result["scene_response"])
...
def apply_turn_result(session_id: str, request: ApplyTurnResultRequest) -> dict:
    manager = SessionManager(); updater = StateUpdater(manager.storage)
    try:
        bundle = manager.get_memory(session_id); _require_active_session(bundle, "applying a turn result")
    ...
    return {"session_id":session_id, "status":result["status"], "message_to_user":rendered_text, "rendered_text":rendered_text, "must_show_to_user":True, "applied":result["applied"], "rejected":result["rejected"], "next_builder_hints":result["next_builder_hints"]}
