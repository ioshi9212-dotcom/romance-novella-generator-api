    ...
    def apply_scene_response(self, session_id: str, scene_response: dict[str, Any]) -> dict[str, Any]:
        ...
        for patch in updates.get("knowledge_patches", []):
            ...
            if not character_id:
                ...
                continue
            base = knowledge.setdefault(character_id, {"knows": [], "does_not_know": [], "must_not_assume": []})
        ...
        for patch in updates.get("new_or_updated_characters", []):
            ...
            if not character_id:
                ...
                continue
            if character_id in characters and characters[character_id].get("locked") and patch.get("replace_locked_card"):
                rejected.append({
                    "target": f"characters.{character_id}",
                    "reason": "locked character card cannot be replaced",
                    "severity": "error",
                })
                continue
            characters[character_id] = {**characters.get(character_id, {}), **patch, "locked": patch.get("locked", True)}
