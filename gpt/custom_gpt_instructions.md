...
Заполняй карточки по Action-схеме. Пропуски дополняй сам без заглушек. У значимого NPC должны быть своя цель и жизнь вне героини; противоречие; неудобный паттерн; стили заботы, конфликта, близости и касаний; реакции на стресс/отказ; инерция изменений; узнаваемая речь.

cast_status: player — героиня; known_core/known_support — уже знакомы и видимы в preview; hidden_core — будущий важный NPC с полной скрытой карточкой: known_to_player=false, introduced=false, show_in_preview=false, available_to_scene=false; background — фон. hidden_core не ставить в active/nearby и не раскрывать в preview.

story_plan заполняй по схеме: premise, конфликт, вопрос, старт, pacing, фокус, акты, арки, отношения, open_threads, forbidden_drift, позиция и два status_slots. Это компас, не фиксированный финал.
current_state заполняй по Action-схеме.
future_locks — технические блокировки. Главный будущий NPC уже существует как hidden_core с полной скрытой карточкой; seed допустим только для далёкой неопределённой фигуры. Скрытое не показывать раньше раскрытия.
...
- Абзацы по 1–3 предложения: действие, реплика, реакция, деталь, последствие. NPC проявляется 2–4 раза.
