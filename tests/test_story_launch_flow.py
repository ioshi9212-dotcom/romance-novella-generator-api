...
def test_player_edits_replace_the_pending_preview_before_one_final_commit():
    client = TestClient(app)
    created = client.post(
        "/api/v1/sessions",
        json={"raw_start_text": "Героиня Мира; остальное придумай сама.", "mode": "gpt_actions"},
    )
    assert created.status_code == 200, created.text
    session_id = created.json()["session_id"]

    first_bootstrap = _valid_bootstrap()
    first = client.post(
        f"/api/v1/sessions/{session_id}/bootstrap-preview",
        json={"bootstrap_json": first_bootstrap},
    )
    assert first.status_code == 200, first.text
    assert first.json()["status"] == "bootstrap_review_pending"

    revised_bootstrap = deepcopy(first_bootstrap)
    revised_bootstrap["characters"]["pc_01"]["name"] = "Mira Vale"
    revised_bootstrap["characters"]["pc_01"]["display_name"] = "Мира Вейл"
    second = client.post(
        f"/api/v1/sessions/{session_id}/bootstrap-preview",
        json={"bootstrap_json": revised_bootstrap},
    )
    assert second.status_code == 200, second.text
    assert second.json()["status"] == "bootstrap_review_pending"
    assert "Мира Вейл" in second.json()["message_to_user"]
    ...
    characters = SessionManager().storage.read_characters(session_id)
    assert characters["pc_01"]["name"] == "Mira Vale"
    assert characters["pc_01"]["display_name"] == "Мира Вейл"
