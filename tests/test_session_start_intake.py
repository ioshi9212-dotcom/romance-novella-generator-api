...
def test_large_multi_character_setup_is_saved_exactly_before_bootstrap():
    character_names = [
        "Елена Высоцкая",
        "Эйден Вернер",
        "Лиам Вернер",
    ...
    raw_start_text = (
        "По ту сторону Аверна\n"
        "Персонажи: "
        + ", ".join(character_names)
        + "\n"
        + "Точный пользовательский канон, правила, отношения и биографии.\n" * 2_000
    )
    ...
    assert body["status"] == "bootstrap_pending"
    assert body["session_id"]
    assert body["bootstrap_prompt_chunk_count"] > 1
    assert body["has_more_bootstrap_prompt_chunks"] is True
    assert len(response.content) < 7_000

    stored = SessionManager().storage.read_json(body["session_id"], "user_request.json")
    assert stored["raw_start_text"] == raw_start_text
    assert all(name in stored["raw_start_text"] for name in character_names)
...
def test_questionnaire_visibly_offers_random_generation():
    ...
    assert "обычным текстом" in body["questionnaire"]
    assert "Railway" in body["questionnaire"]
    assert "preview без спойлеров" in body["questionnaire"]
    assert "До подтверждения игра не активируется" in body["questionnaire"]
...
def test_custom_gpt_passes_the_exact_questionnaire_answer_to_create_session():
    ...
    assert 'createSession(raw_start_text="<все точные ответы пользователя по порядку>"' in instructions
    assert "отдельное подтверждение анкеты не нужно" in instructions
    assert "Это единственное подтверждение перед игрой" in instructions
