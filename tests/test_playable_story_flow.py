...
def test_railway_assembles_the_complete_persisted_context_for_each_turn():
    ...
    assert "RUNTIME_SCENE_RULES" in prompt
    assert "SCENE_CONTRACT_JSON" in prompt
    for section in (
        '"current_frame"',
        '"loaded_characters"',
    ...
    assert "Эмили Харпер" in prompt
    assert "Ethan Cole" in prompt
    assert "future_stranger_seed" not in prompt
