...
def test_create_session_repairs_fillable_character_and_location_omissions(
    ...
    response = client.post("/api/v1/sessions", json=incomplete_character)
    assert response.status_code == 200, response.text
    assert response.json()["creation_verified"] is True
    assert response.json()["normalization_applied"] is True
    stored_card = service.storage.read_json(
        response.json()["session_id"], "characters/char_emily/card.json"
    )
    assert stored_card["appearance"]["distinguishing_details"]

    incomplete_location = deepcopy(session_payload)
    incomplete_location["locations"][0]["state"]["canon"].pop("layout")
    response = client.post("/api/v1/sessions", json=incomplete_location)
    assert response.status_code == 200, response.text
    location_id = response.json()["session_id"]
    stored_location = service.storage.read_json(location_id, "locations/loc_home.json")
    assert stored_location["canon"]["layout"]
...
def test_openapi_requires_current_contract_but_runtime_keeps_legacy_compatibility() -> (
    None
):
    app.openapi_schema = None
    schema = app.openapi()
    ...
    assert request_schema["properties"]["runtime_contract_version"]["enum"] == ["2.0"]


def test_current_contract_repairs_empty_director_plan(
    client, service, session_payload
) -> None:
    ...
    stored = service.storage.read_json(
        accepted.json()["session_id"], "state/director_plan.json"
    )
    assert stored["active_threads"]
    assert stored["character_agendas"]
...
def test_validation_error_does_not_echo_large_rejected_payload(
    ...
    invalid["characters"] = []
    invalid["untrusted_blob"] = "x" * 100_000
    ...
    assert body["error"]["issues"][0]["location"] == "body.characters"
