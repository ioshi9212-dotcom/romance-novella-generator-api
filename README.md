...
- Перед каждой сценой GPT получает актуальные rules, builder, state, карточки,
  индивидуальные знания, отношения, хронологию и полные ходы текущего цикла.
...
ошибку, пока audit-шлюз не закрыт.
...
4. Не добавлять API key и не включать методы списка/последней сессии.
5. Начать новый чат словом «начнём», проверить превью и только затем написать
   «подтверждаю».
